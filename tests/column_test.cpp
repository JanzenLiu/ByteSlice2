...
protected:
    WordUnit* data_;
    const size_t num_ = 12.5*kNumTuplesPerBlock;
    const size_t bit_width_ = 21;
    const WordUnit mask_ = (1ULL << bit_width_) - 1 ;
};

TEST_F(ColumnTest, DISABLED_LoadTextFile){
    std::string filepath = "columntest.tmp";
...
TEST_F(ColumnTest, NaiveBulkLoadAndScanLiteral){
    ...
    Column* column = new Column(ColumnType::kBitSlice, bit_width_, num_);
