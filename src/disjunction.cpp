...
void Disjunction::AddPredicate(AtomPredicate predicate){
...
void Disjunction::ExecuteBlockwise(BitVector* bitvector){
    ...
    for(size_t block_id = 0; block_id < num_blocks; block_id++){
        ...
        for(size_t pid = 0; pid < conjunctions_.size(); pid++){
            ...
            block->Scan(conjunctions_[pid].comparator,
                        ...
                        bmblk,
                        (0 == pid)? Bitwise::kSet : Bitwise::kOr);
        }
...
void Disjunction::ExecuteColumnwise(BitVector* bitvector){
...
void Disjunction::ExecuteNaive(BitVector* bitvector){
...
void Disjunction::ExecuteStandard(BitVector* bitvector){
    //Execute predicates one by one
    for(size_t pid = 0; pid < conjunctions_.size(); pid++){
        //multi-thread is already contained in it
        conjunctions_[pid].column->Scan(conjunctions_[pid].comparator,
