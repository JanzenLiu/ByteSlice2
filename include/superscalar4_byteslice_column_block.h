#ifndef SUPERSCALAR4_BYTESLICE_COLUMN_BLOCK_H
#define SUPERSCALAR4_BYTESLICE_COLUMN_BLOCK_H
...
static constexpr size_t kMemSizePerSuperscalar4ByteSlice = 
    sizeof(ByteUnit)*CEIL(kNumTuplesPerBlock, kNumAvxBits/8)*(kNumAvxBits/8);

/*
   in the class name, *4* means using 256-bit SIMD to simulate 256*4-bit SIMD
 */
template <size_t BIT_WIDTH, Direction PDIRECTION = Direction::kRight>
class Superscalar4ByteSliceColumnBlock: public ColumnBlock{
public:
    Superscalar4ByteSliceColumnBlock(size_t num=kNumTuplesPerBlock);
    virtual ~Superscalar4ByteSliceColumnBlock();
...
private:
    ...
    template <Comparator CMP>
    void ScanHelper1(const Superscalar4ByteSliceColumnBlock<BIT_WIDTH, PDIRECTION>* other_block,
                            BitVectorBlock* bvblock, Bitwise bit_opt) const;
    template <Comparator CMP, Bitwise OPT>
    void ScanHelper2(const Superscalar4ByteSliceColumnBlock<BIT_WIDTH, PDIRECTION>* other_block,
...
inline WordUnit Superscalar4ByteSliceColumnBlock<BIT_WIDTH, PDIRECTION>::GetTuple(size_t pos) const{
    WordUnit ret = 0ULL;
...
template <size_t BIT_WIDTH, Direction PDIRECTION>
inline void Superscalar4ByteSliceColumnBlock<BIT_WIDTH, PDIRECTION>::SetTuple(size_t pos, WordUnit value){
    switch(PDIRECTION){
        case Direction::kRight:
